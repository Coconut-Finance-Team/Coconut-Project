...
public class KISWebSocketClient {
        ...
        try {
            ...
            client.execute(new AbstractWebSocketHandler() {
                ...
                public void afterConnectionEstablished(WebSocketSession session) throws Exception {
                    ...
                    System.out.println("[StockWebSocketService] WebSocket 연결 성공");
    ...
    private void processIndexData(String redisKey, String marketName, String[] values) {
        try {
            ...
            redisTemplate.opsForList().leftPush(redisKey, json);
            System.out.println("[StockWebSocketService] " + marketName + " 데이터 저장: " + stockData);
        } catch (Exception e) {
            System.err.println("[StockWebSocketService] " + marketName + " 데이터 처리 오류: " + e.getMessage());
        }
    }
    ...
    private void reconnectWithBackoff() {
        ...
        for (int i = 0; i < 5; i++) {
            try {
                ...
                System.err.println("[StockWebSocketService] 재연결 대기 중 오류: " + e.getMessage());
            }
            backoff *= 2;
        }
        System.err.println("[StockWebSocketService] WebSocket 재연결 실패: 최대 시도 초과");
    }
